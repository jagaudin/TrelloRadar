...
class AuthForm:
    ...
    class FormBrowser(WinForms.Form):
        ...
        token_url = 'https://trello.com/1/authorize?key=%s&name=%s&expiration=%s&response_type=token&scope=%s'
        
        token_success_string = "You have granted  access to your Trello information."
...
class TrelloToDoApp():
    ...
    def get_config(self):
        ...
        if self.config.has_option("search","search strings"):
            self.search_strings = self.config["search"]["search strings"].split(";")
        else:
            self.config['search'] = {}
    ...
    def on_closing(self):
        self.config["search"]["search strings"] = ';'.join(s for s in self.search_strings)
        self.config.write(self.config_path.open('w'))
        self.root.destroy()
